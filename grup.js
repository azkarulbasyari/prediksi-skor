...
/* ---------- selectors di dashboard ---------- */
function initSelectors() {
  const matchSelect = $("groupMatchSelect");
  matchSelect.innerHTML = matches
    .map(
      (match) =>
        `<option value="${match.id}">${match.flagA} ${match.teamA} vs ${match.flagB} ${match.teamB} — ${match.time}</option>`,
    )
    .join("");
  $("championSelect").innerHTML = countries
...
function updateSelectedMatch() {
  const match =
    matches.find(
      (item) => String(item.id) === String($("groupMatchSelect").value),
    ) || matches[0];
...
/* --------- masuk ke grup & render dashboard ---------- */
async function enterGroup(code, memberName, { silent } = {}) {
  try {
    ...
    console.error(err);
    toast(
      "Gagal konek ke server. Cek koneksi internet / konfigurasi Firebase.",
    );
    return false;
  }
}
...
function renderLeaderboard(members, predictions, champions) {
  ...
  predictions.forEach(
    (item) => (points[item.memberName] = (points[item.memberName] || 0) + 5),
  );
  champions.forEach(
    (item) => (points[item.memberName] = (points[item.memberName] || 0) + 10),
  );

  const ranks = Object.entries(points).sort((a, b) => b[1] - a[1]);
  if (ranks.length === 0) {
    $("groupLeaderboard").innerHTML =
      '<div class="empty-box">Belum ada anggota.</div>';
    return;
  }

  const medals = ["🥇", "🥈", "🥉"];
  $("groupLeaderboard").innerHTML = ranks
    .map(
      ([name, point], index) => `
    <div class="rank-item">
...
$("createGroupForm").addEventListener("submit", async (event) => {
  ...
  try {
    ...
    $("createdInfo").textContent =
      `Grup ${groupName} berhasil dibuat. Bagikan kode ini ke teman, lalu mereka bisa join dari HP/laptop masing-masing.`;
...
$("groupScoreForm").addEventListener("submit", async (event) => {
  ...
  try {
    await dbSet(
      `groups/${activeGroupCode}/predictions/${memberKey}_${match.id}`,
      {
